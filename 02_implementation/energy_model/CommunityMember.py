...
class CommunityMember(mesa.Agent):
    ...
    def update_data(self):
        ...
        self.D_bl = np.array(self.model.database.qh_residential_load_profiles[self.residential_type][ti])
        ...
        self.price_g_d = np.array(self.model.database.qh_electricity_price_kWh['price_ct_kWh'][ti])
        self.price_g_s = np.repeat(self.model.database.optimization_parameter['fit_tariff'], len(ti))
    ...
    def setup_hems(self):
        ...
        # decision variables
        self.g_d = self.m.addVars(rti, vtype=GRB.CONTINUOUS, name='g_d', lb=0, ub=self.variable_bounds)
        self.g_s = self.m.addVars(rti, vtype=GRB.CONTINUOUS, name='g_s', lb=0, ub=self.variable_bounds)
        ...
        self.n_d = self.m.addVars(rti, vtype=GRB.CONTINUOUS, name='n_d', lb=0, ub=self.variable_bounds)
        self.n_s = self.m.addVars(rti, vtype=GRB.CONTINUOUS, name='n_s', lb=0, ub=self.variable_bounds)
        ...
        self.pv = self.m.addVars(rti, vtype=GRB.CONTINUOUS, name='pv', lb=0, ub=self.variable_bounds)
        self.hb_c = self.m.addVars(rti, vtype=GRB.CONTINUOUS, name='hb_c', lb=0, ub=self.hb_c_max)
        ...
        for j in rti:
            ...
            if self.prosumer:
                ...
                self.m.addConstr(-(self.big_M * self.pv_sur[j]) + self.pv[j] <= self.D_bl[j], name=f'pv_sur_t_{j}')
                ...
                self.m.addConstr((self.pv_sur[j] == 1) >> (self.g_d[j] <= 0), name=f'pv_sur_t_{j} -> g_d = 0')
                self.m.addConstr((self.pv_sur[j] == 1) >> (self.n_d[j] <= 0), name=f'pv_sur_t_{j} -> n_d = 0')
                self.m.addConstr((self.pv_sur[j] == 0) >> (self.g_s[j] <= 0), name=f'pv_sur_f_{j} -> g_s = 0')
                self.m.addConstr((self.pv_sur[j] == 0) >> (self.n_s[j] <= 0), name=f'pv_sur_f_{j} -> n_s = 0')
                # grid: can either be sold to or bought from at any time step
                self.m.addConstr(self.g_d_binary[j] * self.big_M >= self.g_d[j], name=f'g_d_bin_{j}')
                self.m.addConstr(self.g_s_binary[j] * self.big_M >= self.g_s[j], name=f'g_s_bin_{j}')
                self.m.addConstr(self.g_d_binary[j] + self.g_s_binary[j] <= 1, name=f'g_d_bin_{j} + g_s_bin_{j}')
                # if p2p trading enabled: Neighbors can either be sold to or bought from at any time step
                if self.model.p2p_trading:
                    self.m.addConstr(self.n_d_binary[j] * self.big_M >= self.n_d[j], name=f'n_d_bin_{j}')
                    self.m.addConstr(self.n_s_binary[j] * self.big_M >= self.n_s[j], name=f'n_s_bin_{j}')
                    self.m.addConstr(self.n_d_binary[j] + self.n_s_binary[j] <= 1, name=f'n_d_bin_{j} + n_s_bin_{j}')
                else:
                    self.m.addConstr(self.n_d[j] <= 0, name=f'p2p_f -> prosumer -> n_d = 0_{j}')
                    self.m.addConstr(self.n_s[j] <= 0, name=f'p2p_f -> prosumer -> n_s = 0_{j}')
                # home battery: can either be charged or discharged at any time step
                self.m.addConstr(self.hb_c_binary[j] * self.big_M >= self.hb_c[j], name=f'hb_c_bin_{j}')
                self.m.addConstr(self.hb_d_binary[j] * self.big_M >= self.hb_d[j], name=f'hb_d_bin_{j}')
                self.m.addConstr(self.hb_c_binary[j] + self.hb_d_binary[j] <= 1, name=f'hb_c_bin_{j} + hb_d_bin_{j}')
            ...
            # consumer constraints: no pv, no storing and selling of energy
                ...
                self.m.addConstr(self.pv[j] <= 0, name=f'consumer -> pv = 0_{j}')
                self.m.addConstr(self.hb_c[j] <= 0, name=f'consumer -> hb_c = 0_{j}')
                self.m.addConstr(self.hb_d[j] <= 0, name=f'consumer -> hb_d = 0_{j}')
                self.m.addConstr(self.hb_soc[j] <= self.hb_soc_t0, name=f'consumer -> hb_soc = 0_{j}')
                self.m.addConstr(self.g_s[j] <= 0, name=f'consumer -> g_s = 0_{j}')
                if not self.model.p2p_trading:
                    self.m.addConstr(self.n_d[j] <= 0, name=f'p2p_f -> consumer -> n_d = 0_{j}')
                self.m.addConstr(self.n_s[j] <= 0, name=f'consumer -> n_s = 0_{j}')

            # consumer and prosumer constraints: ev and load flexibility
            # no ev: no ev (dis-)charging
            if self.nr_evs < 1:
                self.m.addConstr(self.ev_c[j] <= 0, name=f'no ev -> ev_c_{j} = 0')
                self.m.addConstr(self.ev_d[j] <= 0, name=f'no ev -> ev_d_{j} = 0')
                self.m.addConstr(self.ev_soc[j] <= self.ev_soc_t0, name=f'no ev -> ev_soc_{j} = 0')
                self.m.addConstr(self.l_ev[j] <= 0, name=f'no ev -> l_ev_{j} = 0')
            else:
                # ev: can either be charged or discharged at any time step
                self.m.addConstr(self.ev_c_binary[j] * self.big_M >= self.ev_c[j], name=f'ev_c_bin_{j}')
                self.m.addConstr(self.ev_d_binary[j] * self.big_M >= self.ev_d[j], name=f'ev_d_bin_{j}')
                self.m.addConstr(self.ev_c_binary[j] + self.ev_d_binary[j] <= 1, name=f'ev_c_bin_{j} + ev_d_bin_{j}')
    ...
    def update_hems(self):
        ...
        if self.nr_evs > 0:
            ...
            if self.model.schedule.steps > 0:
                self.m.setAttr('RHS', self.m.getConstrByName(f'ev_soc_update_0'), self.ev_soc_t - self.D_ev[0])
    ...
    def get_hems_results(self):
        ...
        res['pv_sur'] = [i.X for i in self.m.getVars() if 'pv_sur[' in i.VarName][0]
        res['hb_c'] = [i.X for i in self.m.getVars() if 'hb_c[' in i.VarName][0]
        res['hb_d'] = [i.X for i in self.m.getVars() if 'hb_d[' in i.VarName][0]
        res['hb_soc_t'] = [i.X for i in self.m.getVars() if 'hb_soc_t[' in i.VarName][0]
