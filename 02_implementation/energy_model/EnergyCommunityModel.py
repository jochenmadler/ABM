...
def total_grid_demand(model):
    return np.sum([a.g_d for a in model.schedule.agents])


def total_grid_supply(model):
    return np.sum([a.g_s for a in model.schedule.agents])


def total_neighborhood_demand(model):
    return np.sum([a.n_d for a in model.schedule.agents])


def total_neighborhood_supply(model):
    return np.sum([a.n_s for a in model.schedule.agents])
...
def pv_generation_actual(model):
    return np.sum([a.PV_max for a in model.schedule.agents])
...
def total_hb_charging(model):
    return np.sum([a.hb_c for a in model.schedule.agents])


def total_hb_discharging(model):
    return np.sum([a.hb_d for a in model.schedule.agents])
...
def compute_avg_self_sufficiency(model):
    return np.mean([a.pv / a.D_bl if a.D_bl > 0 else a.pv for a in model.schedule.agents])
...
class EnergyCommunityModel(mesa.Model):
    ...
    def calculate_p2p_price_levels(self):
        ...
        price_n_max = [i - 0.01 * i for i in price_g_d]
        price_n_min = [i + 0.01 * i for i in
                       [price_g_s[i] + self.database.optimization_parameter['grid_fees'] for i in self.time_index]]
        ...
        n_price_levels.index = price_g_d.index
    ...
    def step(self):
        ...
        for i in range(self.nr_n_equilibrium_price_levels):
            ...
            self.n_s_price_t = pd.Series(
                [self.n_d_price_t[i] - self.database.optimization_parameter['grid_fees'] for i in
                 self.time_index], index=self.n_d_price_t.index)
            # run agents' optimization, track results, and cumulate p2p buying/selling volume
            a_n_d_volumes = a_n_s_volumes = [0] * len(self.time_index)
            for a in self.schedule.agents:
        ...
        # obtain p2p trading price level that maximizes p2p trading volume and store corresponding hems_results
        max_n_index = max(self.n_welfare, key=self.n_welfare.get)
        for a in self.schedule.agents:
            a.store_hems_result(max_n_index)

        # for current time step, obtain total p2p trading demand/supply and calculate max. p2p trading volume
        ...
        # market clearing
        self.schedule.step()
        # collect data for model and agents and shift time_index forwards by one
